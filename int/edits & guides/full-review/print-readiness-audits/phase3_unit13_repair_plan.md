...
Unit 13 is structurally stable at the visible-shell level:

- visible sequence is `A B C D E F G H`
- div balance is `16 opens / 16 closes`
- placeholders total `19`
...
`Explanation for Two Readers` and `Homework Task` are not simple duplicates:

- the first asks students to control detail for two internal readers
- the second asks them to rewrite a real explanation or a new scenario explanation using the unit structure
...
- the audience-control purpose of `Explanation for Two Readers`
- the real-text or scenario rewrite purpose of the homework task

### 4. Warm-up placeholders should be rechecked

The `U13-warmup-missing-*` boxes are all `rows=1`. They may be adequate, but their fit should be considered after the warm-up wording is finalized.
...
1. Keep `C-F` aligned with the current live tasks.
2. Expand `G` guidance so it reflects the actual reflection prompts.
3. Expand `H` guidance so both the two-reader transfer task and the homework rewrite task are explicitly covered.
