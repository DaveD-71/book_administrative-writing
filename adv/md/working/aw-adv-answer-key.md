# Administrative Writing — Advanced: Answer Key
...
### A. Opening & Activation — Identifying Confusing Sentences

**Sentence 1:** *We confirmed the applicant submitted updated documents that were incomplete because the translation team had not finished their review although they intended to submit a draft earlier which caused delays.*

- **Main idea:** We confirmed that the documents were incomplete.
- **Secondary ideas:** reason (translation team hadn't finished), concession (they intended to submit earlier), consequence (delays).
- **Source of confusion:** Three relative/subordinate clauses are stacked without clear segmentation. The reader loses track of the main subject-verb by the end.

**Sentence 2:** *The meeting participants agreed to revise the guidelines which were noted by several offices to be inconsistent with previous instructions that require further study.*

- **Main idea:** Participants agreed to revise the guidelines.
- **Secondary ideas:** evaluation by offices (inconsistent with previous instructions), need for further study.
- **Source of confusion:** The relative clause "which were noted..." embeds a judgment inside the main decision. The phrase "that require further study" is ambiguous — does it refer to the guidelines or to the previous instructions?

**Sentence 3:** *Since the system update was completed last month but the manual was not updated, staff reported errors that made processing applications difficult which resulted in delays.*

- **Main idea:** Staff reported errors that caused delays.
...
2. *The team attempted to submit the report,* **but** *the system outage prevented completion.*
...
1. *Because the file was corrupted, the attachments did not open.* — **Reason**
...
**1.** *Although we received the applicant's correction, we still need to confirm whether the previous version remains valid because the internal database has not been updated.* → **Segmentation:**
> We received the applicant's correction. However, we still need to confirm whether the previous version remains valid, as the internal database has not yet been updated.

**2.** *Since several offices raised concerns about the inconsistency, we will draft a revised explanation to ensure alignment.* → **Pattern 1 (main idea → reason):**
> We will draft a revised explanation to address the inconsistency raised by several offices. / We will issue a revised explanation because several offices raised concerns about the inconsistency.

**3.** *We must confirm whether the translation is final, and we must also check whether the correct version was submitted, and this is needed before the examination can proceed.* → **Segmentation:**
> We must confirm whether the translation is final and whether the correct version was submitted. Both points need to be resolved before the examination can proceed.
...
- **Problem:** "which" is ambiguous — it could refer to our informing them, or to the report being incorrect.
- **Revision:** We informed the office that the report was incorrect. This notification caused some confusion. / The office was confused when we notified them that the report contained an error.

**2.** *They updated the database after reviewing the submitted documents that contained several errors.*

- **Problem:** "that contained several errors" could modify "database" or "documents."
- **Revision:** After reviewing the submitted documents, which contained several errors, they updated the database. / They found several errors in the submitted documents and then updated the database accordingly.
...
| Phrase | Function |
|--------|---------|
| A. *It appears that…* | 3. Introducing a possible issue without assigning blame |
...
1. *Why did you change the format?* → Could you clarify the reason for the recent format change? / We would like to understand the background to the format adjustment. Could you provide some context?

2. *When will you finish the revision?* → Could you advise on the expected timeline for the revision? / Would you be able to confirm when the revised version will be available?

3. *Who made this decision?* → Could you confirm which office or individual authorised this decision? / For our records, we would appreciate knowing the source of this decision.
...
1. *You did not attach the file.* → It appears that the attachment may not have been included in the most recent message. Could you resend it at your convenience?
   - **Softening strategy:** *It appears that...* (removes direct accusation); modal hedge (*may not have been*)

2. *This mistake caused significant delay.* → The discrepancy in the submitted version has resulted in a delay to the review timeline.
   - **Softening strategy:** Passive voice (*has resulted in*); nominalization (*discrepancy* removes personal blame); *mistake* replaced by neutral *discrepancy*

3. *We already told you to revise this.* → As mentioned in our previous correspondence, this section requires revision before the file can proceed.
   - **Softening strategy:** *As mentioned in our previous correspondence* (neutral reference, not accusatory); *requires revision* instead of "you need to"
...
### A. Opening — Classify Excerpts by Cohesion Type

| # | Type | Reason |
|---|------|--------|
| 1 | **Disjointed** | Five unrelated facts with no logical connection between them |
| 2 | **Cohesive** | Each sentence builds on the previous; connectors ("To address these") signal purpose |
| 3 | **Disjointed** | Four short declarative sentences with no connectors; topics shift abruptly |
| 4 | **Cohesive** | "Because" links cause to effect; second sentence shows result and outcome |
| 5 | **Disjointed** | Reads like a sequence of events but lacks connectors; "The team reviewed" to "Approval was given" jumps without explanation |
| 6 | **Cohesive** | "Several errors… returned for correction" → "once reviewed and approved… forwarded" — clear cause-and-effect chain |
...
1. *The document contains errors. **It** must be corrected before submission.* / *The document contains errors and must be corrected before submission.*

2. *The applicants raised several questions. **These** will be addressed at the next meeting.*

3. *The office submitted the revision late. **This delay** caused problems for the review team.*
...
2. *The review was completed.* **However**, *two errors remain.*

3. *The instructions were updated.* **Nevertheless** / **However**, *some teams are still using the previous version.*
...
**Information provided:** (see student book exercise)

**Model two-paragraph version:**

> The coordination meeting took place on 14 March, during which three key issues were identified. The issues related to terminology, submission formats, and internal deadlines. Offices were asked to respond by 20 March; however, confirmation has not yet been received from two of the participating offices.
>
...
**Key differences (revised vs. original):**

| Feature | Original | Revised |
|---------|---------|---------|
| Greeting | "Hi" | Formal opening: "Thank you for your message" |
| Request | Vague, combined | Numbered, separated |
| References | None | "Section 3.1", "Annex A" |
...
**Original:** "Hi, we saw your message. Can you send the other file? Also, the translation had some issues so please check that too. By the way, we need to finish the review soon."

**Model rewrite:**

> Subject: Follow-Up — Updated File and Translation Review
>
> Dear [Name],
>
> Thank you for your message.
>
> We are writing to follow up on two points. First, could you confirm whether the updated file mentioned in your message is now available? Second, we noted several inconsistencies in the translation submitted with the file. We would appreciate your review of those sections before the next stage proceeds.
>
> If possible, we would welcome a response by [date], as the review timeline depends on receiving both items.
>
> Kind regards,
> [Name]
...
1. *Can you explain the date issue?* → In Section [X], the date listed appears to differ from [Annex / earlier version]. Could you confirm which date should be used?

2. *What does "response" mean here?* → In Section 2.3, the term "response" is used. Could you clarify whether this refers only to written responses, or whether it also includes verbal or electronic communications?
...
|--------------------------|----------------------|
| For our confirmation, we would appreciate clarification on… | In Section 3.1 of the attached document… |
| To ensure accuracy, may we confirm whether… | According to the table in Annex B… |
| We would be grateful if you could clarify… | The term used on page 4… |
| It appears that… | There may be a discrepancy regarding… |

| Formulating questions | Next steps |
|----------------------|-----------|
| Could you confirm which version is correct? | Once confirmed, we will proceed with… |
...
1. *The date you provided is incorrect.* → Upon review, it appears that the date in [Section X] may differ from our internal records. We would appreciate your confirmation of the correct date.

2. *You attached the wrong version of the form.* → Upon review, the form received may not be the most recent version. We would appreciate it if you could confirm which version is currently valid.

3. *The term is used inconsistently throughout the document.* → There may be an inconsistency in the terminology used across sections of the document. For our internal confirmation, we would appreciate your guidance on the intended term.

4. *The translation contains errors.* → Upon review, it appears that certain sections of the translation may differ from the source document. We would appreciate clarification before we proceed.
...
|---|---|---|
| 1 | Blame assignment (*because you made a mistake*) | *We have identified a discrepancy in the submitted document that is preventing us from proceeding. We would appreciate your assistance in resolving this at your earliest convenience.* |
| 2 | Blame assignment (*your file is causing delays*) | *A discrepancy in the submitted file has affected our processing timeline. To ensure the review can continue, we would appreciate your confirmation of the correct version.* |
| 3 | Missing reference (*check it* — too vague) | *Could you confirm whether the information in [Section X] is correct, as it may differ from [reference document / earlier version]?* |
| 4 | Blame + urgency (*you need to fix it immediately*) | *There appears to be an inconsistency in the document that may affect the next stage of review. We would appreciate your assistance in confirming and correcting this point.* |
...
**Sentences:** *You used the wrong version. / This error caused delays. / We can't continue until you fix the problem. / Your explanation doesn't make sense.*
...
### A. Opening — Count and Sequence Requests

1. *"Please confirm the applicant's address and resend the updated file."* — **2 requests.** Both are equally clear and parallel; no dependency is stated between them.
2. *"Could you clarify the date... and verify whether the term aligns...? Also, if an updated version is available, please share it."* — **3 requests.** The date clarification and terminology check are roughly equal in importance; the updated-version request is conditional (*if available*). The implicit sequence (clarify → check → share) is present but unstated.
3. *"We would appreciate it if you could review the attached document."* — **1 request**, but it is vague — "review" does not specify what the reader should look for or respond to.
...
**Original:** *"We need to sort out a few things before we can move forward. The terminology in the document needs to be checked. Also, the timeline should be confirmed... The translated version also needs a review, but we can't start that until we know which terms are being used. Once everything is clear, we can update the file and proceed."*

**Model rewrite:**
> To proceed with the review, we would appreciate your assistance with the following points, in the order below.
>
> First, could you confirm which terminology should be used throughout the document? This is the critical first step, as the translated version cannot be reviewed until the correct terms have been established.
>
> Once the terminology has been confirmed, we would be grateful if you could advise on the correct timeline — in particular, which date should be treated as the operative deadline for this stage.
>
> Finally, once both points above have been resolved, we would appreciate your review of the attached translation for consistency with the agreed terminology.
>
...
**Model rewrite:**
> To ensure the review can proceed accurately, we would appreciate your assistance with the following steps.
>
> First, could you confirm which version of the document is currently valid? This will help us identify which dates and terminology should apply.
>
...
1. **Steps identified:**
   ...
   2. If information is missing → send request within two working days.
   3. Once complete → forward to reviewing officer.
   4. If reviewing officer finds inconsistency → return to originating office with written explanation.
   5. If accurate → proceed to approval stage.
   *(Note: "processing times may vary" is a caveat, not a step.)*

2. **Steps with conditions:** Steps 2, 4, and 5 include conditions (*if, once*). Steps 1 and 3 are unconditional.

3. **Sequencing:** Generally clear. One ambiguity: after Step 2 (*send a request*), it is unclear when the process resumes — does it wait for a reply? A "once information has been received" step is implied but not stated.
...
1. **F** — *Log the corrected submission in the internal tracking system upon receipt.* *(First action on receipt)*
2. **D** — *Compare the corrected submission against the original to identify all changes made.* *(Review content before any record update)*
3. **B** — *If the correction resolves the identified issue, update the processing record accordingly.* *(Conditional record update — only when correction is confirmed)*
4. **C** — *Forward the file to the reviewing officer for final assessment.* *(Proceed to next stage)*
5. **E** — *If new discrepancies are found, return the file with a written explanation of the outstanding issues.* *(Handle failure condition before the closing step)*
...
| Original | Rewrite | Verb changed |
|---|---|---|
| 1. *[Do] the initial review…* | *Conduct the initial review before forwarding the file.* | *do* → *conduct* |
| 2. *[Look at] the changes made…* | *Examine the changes made in the revised submission.* | *look at* → *examine* |
...
**Original:** *"First of all, check the document carefully and if something is not right, maybe ask a colleague to have a look too. After you've done that, send whatever reply seems right to the applicant. You also need to update the record at some stage — probably before or after the reply, depending on the situation. Make sure guidelines are being followed somehow. If there are problems, deal with them."*

**Issues:** Vague verbs; no logical sequence (record update is an afterthought); conditions implied but not stated; informal language; no closing step.

**Model revised procedure:**

**Procedure for Reviewing Applicant Documents**

1. **Initial Review:** Examine the submitted document to confirm that all required fields are complete and consistent.
2. **Discrepancy Check:** If any discrepancy or missing information is identified, consult the relevant guidelines to determine the appropriate response.
3. **Record Update:** Update the internal processing record to reflect the current status of the document.
4. **Applicant Notification:** Send the appropriate response to the applicant — either a confirmation of acceptance or a request for correction — in accordance with the applicable guidelines.
...
| Annotation | Section of text |
|---|---|
| 1. *Opens with a formal signal phrase + effective date* | *"Please note that the procedure… has been updated, effective 1 November."* |
| 2. *Describes the change precisely — specifying what is different now* | *"The revised process now requires sign-off from one reviewing officer… replacing the previous requirement for dual sign-off."* |
...
1. *The rules changed on Monday.* → Please note that the revised rules came into effect on Monday, [date]. Staff are asked to ensure that the updated requirements are applied from this date.

2. *We updated the form last week.* → The applicant submission form has been updated, effective [date], to reflect the revised data fields required for all new submissions. The previous version is no longer in use.

3. *Some parts are different now.* → Several key elements of the procedure have changed, replacing the previous approach. In particular, [specific element] has been revised to [brief description of change].

4. *The process is longer.* → The revised process now includes an additional verification step that was not required under the previous version. This change has been introduced to improve accuracy during the review stage.
...
**Original:** *"We changed the system. Please use the new rules from now on. The old form doesn't work anymore so stop using it. We decided this was better because of various reasons. If you're confused, ask someone. The new form is somewhere on the shared drive."*

**Issues:** No effective date; no explanation of what changed; vague rationale (*various reasons*); unhelpful instructions (*ask someone*, *somewhere on the shared drive*); informal tone; no formal support contact.

**Model revised notice:**
> Please note that the procedure for [subject] has been updated, effective [date].
>
> The revised process replaces [the previous form / the previous workflow], which is no longer valid from this date. The key change concerns [specific element]: [brief description]. This revision has been introduced to [reason — e.g., improve consistency across offices / reduce processing delays / align with the updated guidelines issued in month].
>
> All staff are asked to begin using the updated form, which is available on the shared drive under [folder name / path]. Please discontinue use of the previous version from the effective date.
>
...
1. **Missing specific information:** What is unclear? Which part of the review? What information is needed and from whom?
...
| Sentence | Function |
|---|---|
| 1. *The delay occurred as a result of three interconnected issues.* | **A** — Introducing the number of factors |
| 2. *The updated form contains a section that is not compatible with our current processing software.* | **B** — Describing a specific factor |
| 3. *For example, the term "review period" appears with two different meanings in the same document.* | **C** — Adding a supporting detail |
| 4. *Consequently, the verification stage has been postponed until clarification is received.* | **D** — Explaining impact or next steps |
...
2. *The data contains inconsistencies.* → The applicant data submitted by Office B in March contains two conflicting figures for the same case — the summary table uses the value recorded at registration, while the main text uses a revised value that was not formally confirmed.

3. *There are some problems with the timeline.* → The current implementation timeline does not include the two-week training period required before the new form can be adopted. As a result, the rollout date of 1 April may not be achievable for all partner offices.
...
**Original:** *"There are many problems, so we cannot continue. Please fix everything first. The data is not right and also the instructions are confusing which is making things very difficult for our team. We hope this can be resolved quickly."*

**Issues:** No factor count; no specific supporting detail; informal/emotive language (*very difficult*, *we hope*); no next-step structure; blame-adjacent phrasing (*please fix everything*).

**Model rewrite:**
> The review is currently on hold due to two main factors.
>
> First, a number of discrepancies have been identified in the data submitted with the file. In particular, the figures in the summary table do not match those in the supporting documents, and this difference must be resolved before analysis can proceed.
>
> Second, the updated instructions contain ambiguous phrasing in Section 3, where the term "review period" appears to be used with two different meanings. This has created uncertainty about which deadline should apply to the current file.
>
...
> - Office D resolved all two of its agenda items. No items remain outstanding.
...
1. *Office A was obviously confused about the new terminology.* → Office A sought clarification regarding the application of the updated terminology, noting that the current guidance does not distinguish clearly between the two relevant terms.

2. *The deadline is totally unrealistic and everyone knows it.* → The proposed implementation deadline was identified as challenging by several offices, given the preparation time required for system updates and staff training.

3. *One office suggested that maybe the timeline needs looking at again.* → Office B recommended that the implementation timeline be reviewed to ensure it reflects the preparation time required across all offices.

4. *People weren't happy about the lack of communication from the coordinating office.* → Several offices noted that communication from the coordinating office regarding the revised guidelines had been limited and requested more regular updates during the implementation period.
...
**Meeting Summary — Reporting Template Review (5 November)**
...
1. **Background context provided:** Reference number (2024/1147), date of submission (14 January), description of what was submitted (revised supporting documents), current status (no confirmation received). This is important because the recipient must be able to locate the specific case and understand where in the process it stands without asking follow-up questions.

2. **Number of questions:** Three. (i) Whether the documents have been accepted; (ii) whether further materials are required; (iii) the estimated timeframe for the next stage. All three are specific and answerable.

3. **Diplomatic softening examples:** (i) *"Could you advise whether…"* — modal verb + *advise* softens a direct question into a polite inquiry; (ii) *"We would also be grateful for an estimated timeframe…"* — *would be grateful* reduces the impression of demand. These are used to preserve the collaborative tone of the relationship.
...
| Expression | Function |
|---|---|
| 1. *We are writing with regard to…* | **A** — Providing context |
| 2. *Could you confirm whether…?* | **B** — Asking a precise question |
| 3. *We would be grateful if you could…* | **C** — Diplomatic softening |
| 4. *This will allow us to finalise our review by…* | **D** — Signalling next steps |
| 5. *For our internal records…* | **A** — Providing context |
...
| Providing context | Asking a question |
|---|---|
| *We are writing to confirm…* | *Could you confirm whether…?* |
| *For our internal processing…* | *Would you be able to advise whether…?* |
| *With regard to the submission made on…* | *Could you advise if…?* |
| *We would appreciate your assistance in…* | *If available, we would also appreciate…* |

| Softening a request | Signalling next steps |
|---|---|
...
**Original:** *"Dear Office, We need information about the application. What is happening? We sent the documents last month. Please tell us if there is a problem and when we will get an answer. We also want to know if you need anything else. Send a reply soon. Thank you."*

**Issues:** No background reference or case number; questions vague and unanswerable; no date; direct and informal tone; no diplomatic softening; no next-step logic.

**Model revised inquiry:**
> Subject: Request for Clarification — Application Reference [XXXX]
>
> Dear Colleagues,
>
> We are writing to follow up on the supporting documents submitted on behalf of application reference [XXXX] on [date].
>
> As our records do not yet show confirmation of receipt, we would be grateful if you could advise whether the documents have been received and processed. Could you also confirm whether any additional materials are required at this stage?
>
> If available, we would appreciate an estimated timeframe for the next step, as our internal review is due to take place in [timeframe].
>
> Please do not hesitate to contact us should any further information be required on our side. We look forward to your response.
>
> Kind regards,
> [Name]
...
| Month | Incomplete submissions (%) | Average processing time (days) | Terminology queries |
|---|---|---|---|
| January | 12% | 8.4 | 6 |
| February | 19% | 11.2 | 14 |
| March | 24% | 13.7 | 21 |
...
2. *Something might go wrong with the process.* → A potential issue relates to the transition from the previous procedure to the revised one: if staff have not completed training before the effective date, inconsistent application of the new requirements may occur across offices.
...
| Sentence | Error type | Revised version |
|---|---|---|
| 1. *The problem is very bad and could cause a lot of trouble for everyone.* | Vague + emotive | *The identified discrepancy in the submitted data may result in processing delays affecting all participating offices if not addressed before the next review cycle.* |
| 2. *Maybe something might be possibly incorrect with the process, perhaps.* | Over-hedged / incoherent | *There is a possibility that the revised procedure has not been applied consistently across all offices, which may affect the accuracy of current processing outcomes.* |
| 3. *We are very concerned that the situation is quite worrying and serious.* | Emotive + vague | *The current situation presents a risk to the implementation timeline that requires prompt attention to prevent further delays.* |
| 4. *There is a risk of: inconsistency, delays, and confusion.* | Structural — list without explanation | *There is a risk that inconsistent application of the revised procedure may lead to processing delays and uncertainty among staff regarding the correct course of action.* |
...
2. *Everyone will be confused by the new form.* → Staff who have not yet received guidance on the revised form may have difficulty completing it accurately, which could lead to an increase in incomplete submissions during the initial implementation period.

3. *The process is broken.* → The current process may not be functioning as intended in all offices, and a review of implementation steps may be needed to identify where inconsistencies have arisen.
...
1. *Please use the updated form.* →
   - *For staff who attended the briefing:* From [date], please begin using the updated form in place of the previous version.
   - *For partner offices:* The applicant submission form has been updated, effective [date]. All offices are asked to use the new version, available at [location]. The previous version should no longer be submitted.

2. *The instructions have changed.* →
   - *For colleagues familiar with the revision:* The main change to the instructions concerns [specific section/element].
   - *For offices not involved in the earlier review:* The applicant processing instructions have been revised, effective [date], to address inconsistencies identified in the previous version. Key changes include [brief description].

3. *Follow the new timeline from Monday.* →
   - *For teams already briefed:* From Monday, please apply the revised timeline as set out in the updated procedure circulated last week.
   - *For partner agencies:* A revised processing timeline will come into effect from Monday, [date]. For offices not yet familiar with the revision, the new timeline shortens the initial review stage from [X] to [Y] days, effective for all submissions received from that date.
...
**Model rewrite:**
> We would like to confirm that the applicant submission form has been updated, effective [date]. All offices are now asked to use the revised version, which replaces the previous form circulated in [month].
>
> For staff who attended the consultation sessions in [month], the principal change concerns [brief description of key change], as agreed during that process.
>
...
**Original:** *"Everyone already knows about the changes, so use the new process and follow the updated instructions. The form has been revised and the old version is no longer valid. Staff should already have read the guidance. If you haven't, contact someone who has. External offices should also do this. There will be more information later."*

**Issues:** Assumes prior knowledge; no context or reference; dismissive of readers who may not know; no explanation of what changed or why; vague action instructions; informal tone; no next-step clarity.

**Model revised message:**
> We would like to confirm that the applicant processing form has been updated, effective [date]. All offices — both internal and external — are asked to use the revised version from this date.
>
> For staff already familiar with the change: the key revision concerns [specific element] in the submission sequence, as discussed in the [date] briefing.
>
> For partner offices that have not yet received a briefing: the form has been revised to [brief reason — e.g., align with the updated interagency guidelines and reduce the rate of incomplete submissions]. The revised form is available at [location].
>
> The previous form should no longer be used for any submission received after [date]. A full guidance note is available on request. If you have any questions, please contact [office] at [contact details].
>
...
1. **Recommendation:** The office recommends adoption of a standardised terminology protocol for all interagency communications.
2. **Policy reference:** *"In accordance with the revised coordination guidelines issued in November, all partner offices are required to use consistent terminology…"* This is important because it grounds the recommendation in an existing requirement rather than personal preference, making the rationale binding rather than advisory.
...
1. *We should update the instructions.* → The instructions should be updated to align with the revised coordination framework issued in [month], which requires all offices to use standardised language when referring to applicant submission stages.

2. *The form needs to be revised.* → The applicant submission form requires revision to comply with the updated data requirements introduced in [Circular/Guideline reference], which specifies the mandatory fields for all new submissions from [date].

3. *Staff should follow the new process.* → In accordance with the procedural guidance issued in [month], all staff are required to apply the revised process to all cases received after [effective date].
...
|---|---|---|
| 1. Replace *"We think the form should be changed because it is old and confusing"* with *"The form should be revised to align with the updated submission guidelines issued in February, which require standardised data fields across all partner offices."* | **Accept** | Removes personal opinion (*we think*), replaces vague justification (*old and confusing*) with a specific policy reference, and states the objective reason for the change. |
...
### A. Opening — Classify Evaluation Statements

| Balanced & Professional | Unbalanced or Unprofessional |
|---|---|
| 2. *Section 3 provides clearer guidance than the previous version, though some definitions remain ambiguous.* | 1. *The document fails to address several key points and is poorly written.* |
| 4. *The procedural steps are well sequenced; however, the transition between Steps 4 and 5 may cause confusion.* | 3. *The formatting is terrible and the writer did not think about the reader.* |
| 6. *In terms of accuracy, most figures are consistent with the supporting data.* | 5. *Everything in Part B is wrong.* |
| 8. *While the overview is clearly structured, the recommendations section would benefit from greater specificity.* | 7. *Nobody will understand this part.* |
...
|---|---|
| **a.** *Opens with an overall impression without committing to a simple positive or negative verdict* | *"The revised document provides clearer guidance in several sections, particularly the updated chronology table. The structure is generally easier to follow than the previous version."* |
| **b.** *Uses criterion-based language — "in terms of" / "in Section X"* | *"Several definitions remain ambiguous, particularly in Section 3.2."* |
| **c.** *Signals a weakness without using emotional or judgmental language* | *"The transition between Steps 4 and 5 is unclear and may cause misinterpretation."* |
| **d.** *Frames the recommendation as an improvement, not a criticism* | *"Clarifying key definitions and improving the logical flow between steps would significantly enhance overall usability."* |
...
2. *The writer obviously didn't think about clarity.* → In several places, the document assumes a level of background knowledge that may not be shared by all intended readers, which reduces the overall accessibility of the text.

3. *The whole section needs to be redone.* → Section [X] would benefit from significant revision. In particular, the structure does not follow a consistent logical sequence, which makes it difficult for readers to identify the required actions.
...
|---|---|---|
| 1. *The document is quite bad in places, though some bits are fine.* | Vague/informal; partial balance | *The document has some clear strengths — particularly [specific element] — but several sections, especially [X], would benefit from greater precision and more detailed supporting explanation.* |
...
**Text:** *The document is ~~bad~~ in need of significant revision in several areas. The ~~instructions are confusing~~ instructions in Section 2 lack sufficient clarity for new users. ~~Nobody~~ No office will be able to implement this without ~~a lot of~~ additional support. However, the ~~nice~~ clear and well-formatted chronology table is a ~~great~~ useful addition.*

| Change | Accept / Reject | Reason |
|---|---|---|
| *bad* → *in need of significant revision* | **Accept** | Removes subjective label; replaces it with a measurable, professional description of the problem. |
| *instructions are confusing* → *instructions in Section 2 lack sufficient clarity for new users* | **Accept** | Adds criterion (*clarity*), specific location (*Section 2*), and targeted scope (*for new users*) — essential elements of evidence-based evaluation. |
| *Nobody* → *No office* | **Accept** | *Nobody* is informal and absolute; *No office* is formal and fits the institutional register of an administrative evaluation. |
| *a lot of* → *additional* | **Accept** | *A lot of* is informal; *additional* maintains the same meaning in the appropriate register. |
| *nice* / *great* → *clear and well-formatted* / *useful* | **Accept** | Both original words are informal and evaluative; the replacements are criterion-based (*clear*, *well-formatted*) and proportionate (*useful* rather than superlative). |
...
| Office | Forms Processed | Error Rate | Avg Processing Time |
|---|---|---|---|
...
1. *We talked about problems in the process.* → Three implementation concerns were identified during the review, relating to terminology inconsistency, timeline feasibility, and form adoption.

2. *Many people mentioned various issues about the form and some people didn't understand the changes, so they talked about many parts for a long time.* → Several offices raised concerns about the clarity of the revised form, particularly regarding the updated terminology in Section 3 and the revised sequencing requirements.

3. *There have been quite a few issues with delays lately and it's becoming a bigger problem.* → Processing delays have increased over the past quarter across multiple offices, presenting a risk to the overall implementation timeline.
...
**Implications:**
Without intervention, inconsistent application of the revised form is likely to continue, reducing processing accuracy across the network.
...
**Sources:** Source 1 (Office A email, 14 March); Source 2 (Meeting Notes, 17 March); Source 3 (Internal Report, 20 March)

1. **Themes appearing across all three sources:** Processing delays in the online portal submission system; a combination of technical and procedural factors contributing to those delays.

2. **What each source adds:**
   - Source 1: a specific procedural cause (unclear file format instructions).
   - Source 2: a technical cause (portal timeout for files over 5MB) and a contrasting cause from Office C (late supporting documents — not a technical issue).
   - Source 3: quantitative evidence (23% increase in resubmissions) and the observation that both technical and procedural factors are contributing.
...
2. *Office A says the timeline is fine. Office B says it is too short.* → Offices A and B differ in their assessment of the current timeline: Office A reports no difficulties, while Office B considers the proposed schedule unrealistic given its current capacity.

3. *Three offices reported delays.* → Processing delays were reported across three offices, though the causes identified varied — encompassing both technical system limitations and procedural gaps in the guidance provided.
...
**Original:** *"Office A said one thing, Office B said something else, and Office C also had comments. Office A said the instructions were confusing. Office B said the timeline was the problem. Office C said both were issues. Also, Office D didn't say much but they mentioned some delays too. We need to think about these issues because they are all important."*

**Issues:** Source-by-source listing; informal language; no thematic grouping; no concluding insight; weak closing.
...
### C. Language Focus — Simplify Technical References

1. *"Section 7(3) mandates procedural harmonisation in respect of applicant submissions across all member offices."* → All member offices are now required to follow the same procedure when processing applicant submissions.

2. *"Pursuant to Annex 4(b), examining units shall update their internal registers within 30 days of the circular's entry into force."* → Examining offices must update their internal records within 30 days of the date the notice comes into effect.

3. *"Offices must ensure compliance with Article 6(2) regarding the new documentation matrix."* → Offices are required to follow the new documentation requirements as set out in the updated guidelines.
...
1. *"The guidelines have been revised."* → The applicant processing guidelines have been revised and are now in effect. Offices are asked to ensure that staff apply the updated guidelines to all new cases from [date] and discontinue use of the previous version.

2. *"Changes to the timeline have been approved."* → The revised processing timeline has been approved and applies to all submissions received from [date]. Offices should review the updated schedule and adjust their internal workflows accordingly.
...
**Original:** *"Offices shall ensure compliance with Section 9 according to the revised administrative schedule as referenced in Annex 2(c) of the updated framework. It should be noted that non-compliant offices will be subject to Article 7 review procedures. The purpose of this revision is procedural harmonisation vis-à-vis the standardised terminology matrix."*

**Issues:** Unexplained technical references (Section 9, Annex 2(c), Article 7); purpose stated in specialist language (*procedural harmonisation vis-à-vis*); no clear required action; inaccessible to a general staff audience.

**Model revised plain-language version:**
> All offices are now required to follow the updated administrative schedule, which came into effect on [date]. The purpose of this revision is to ensure that all offices use the same terminology when processing applicant files, reducing inconsistencies that have arisen from the use of different terms across the network.
>
> Offices that do not follow the updated schedule may be subject to a formal review by the coordinating unit.
>
...
1. *"We are thinking about changing the guideline."* → The guideline for [subject] will be revised, effective [date].

2. *"Some updates to the process might be needed."* → The office has decided to update the [specific process] to align with the revised requirements issued in [month].

3. *"The form is going to be different going forward."* → The applicant submission form has been revised and will take effect from [date], replacing the previous version.
...
2. *"The deadline was extended."* → The submission deadline has been extended from [original date] to [new date] in response to processing delays identified in the most recent coordination review, and in accordance with the flexibility clause in [relevant guideline]. This adjustment allows offices to submit complete, accurate documentation rather than rushing to meet an unachievable deadline.
...
1. *"The instructions have been updated."* → … This will reduce ambiguity in how staff interpret the requirements and support more consistent application of the process across all partner offices.

2. *"A new reporting template has been introduced."* → … This will enable the coordinating unit to monitor progress across offices more efficiently and identify outstanding action points before they cause delays.

3. *"The review timeline has been shortened."* → … This will reduce applicant waiting times and align the office's performance with the targets set in the updated interagency coordination framework.
...
1. **B** — *The office will adopt the revised applicant terminology list from 1 May.* *(Decision statement)*
...
**Original:** *"We changed the workflow because the old one wasn't very good and people found it confusing. The new one is simpler and should work better. Everyone agreed it needed to be changed so we went ahead with it."*

**Issues:** No specific decision statement; no policy or evidential basis; vague language (*wasn't very good*, *should work better*); informal register (*went ahead with it*); consensus appeal replaces rationale.

**Model revised justification:**
> The office has adopted a revised workflow for processing interagency requests, effective [date].
>
> The previous workflow contained duplicated steps that led to repeated follow-up queries and inconsistent handling of routine cases. This revision aligns with the updated processing standards issued in [reference], which require a streamlined, clearly sequenced procedure for all incoming requests.
>
...
| Office | Briefing | Updated form | Outstanding queries | Status |
|---|---|---|---|---|
| A | Yes | Yes | 2 | On track |
| B | Yes | No | 4 | Delayed |
| C | No | No | 7 | Not started |
| D | Yes | Yes | 0 | On track |
| E | Yes | Yes | 1 | On track |

**Model Issues Identified section:**
> Three offices (A, D, and E) are on track, having completed briefings and adopted the revised form. Office B has completed its briefing but has not yet adopted the updated form, resulting in a delayed status. Office C has not yet completed its briefing or adopted the new form and has not begun implementation. The volume of outstanding queries is concentrated in offices with incomplete or delayed implementation: Office C has submitted seven queries and Office B four, compared with a combined total of three from the three on-track offices.
...
1. *"Some offices don't understand the new instructions."* → Several offices identified uncertainty regarding the application of the updated instructions, particularly in relation to [specific aspect].

2. *"Office B failed to submit their update on time."* → Office B's status update was not received by the indicated deadline. Follow-up will be required to confirm current implementation status.

3. *"The guidelines caused a lot of confusion."* → The revised guidelines gave rise to a number of clarification requests from partner offices, indicating that additional guidance may be required in certain areas.
...
2. *"Something was done about the terminology issue."* → A consolidated terminology clarification was prepared by the coordinating unit and circulated to all offices on [date], addressing the most frequently raised points of ambiguity.

3. *"Offices were contacted."* → The coordinating unit sent follow-up messages to all offices that had not yet submitted their implementation updates, requesting confirmation of current status by [date].
...
2. *"The question about the timeline has not been resolved."* → The question of whether the revised timeline applies to applications already in progress will be referred to [senior body / legal unit] for clarification. A response is expected by [date].
...
| Sentence | Section |
|---|---|
| 6. *This report consolidates implementation feedback from four offices following the distribution of Circular 2024/09.* | **Summary** |
| 3. *Two offices are experiencing delays because the updated forms have not yet been received.* | **Issues Identified** |
| 7. *Several offices requested confirmation of which applicants fall under the revised processing sequence.* | **Issues Identified** |
| 2. *It remains unclear whether the new applicant timeline applies to cases submitted before the circular's entry into force.* | **Outstanding Questions** |
| 1. *Office A completed staff training on the revised terminology on 18 October.* | **Actions Taken** |
| 5. *Office B circulated an internal guidance note to all examining staff.* | **Actions Taken** |
...
**Issues:** No section labels; no specific information; vague language; no attribution; no time-bound next steps; unprofessional tone.

**Model revised report:**
...
### A. Opening — Prediction Task

**Likely document type:** A consolidated administrative notice or coordination report addressed to all examining offices.

**What it would need to include:** (1) A summary of current implementation status; (2) clarification of the terminology question from Office A; (3) acknowledgment of and response to Office B's deadline concern; (4) a reminder about the outdated forms issue; (5) the acknowledgement requirement from Circular 2024/11; (6) clear next steps with deadlines.
...
| Sentence | Problem | Revised version |
|---|---|---|
| 1. *"Office B failed to meet the deadline again, which is creating problems for everyone."* | Blame-heavy; informal | *Office B has indicated that the current deadline may not be achievable given existing staffing constraints. This situation may affect the overall implementation timeline and will be considered on a case-by-case basis.* |
| 2. *"Regarding the terminology issue, it's basically just a misunderstanding that shouldn't have happened."* | Dismissive; informal; blame-adjacent | *Office A has requested clarification on the revised terminology in Section 3. A formal clarification note will be circulated to address this point.* |
| 3. *"We are writing to tell you about various things that need to be sorted out urgently."* | Vague purpose; informal | *This notice summarises current implementation issues arising from the distribution of Circular 2024/11 and sets out the actions required from all offices.* |
...
### B. Example Text — Compare Responses A and B

**How Response B demonstrates the key capstone skills:**

- **Synthesis:** Opens with a purpose statement and groups issues thematically (terminology inconsistency, then form-adoption delays, then unresolved meeting outcomes) rather than addressing each source separately.
- **Neutral, diplomatic tone:** Problems described factually (*"terminology in the revised guidelines is being applied inconsistently"*; *"continue to use a previous version"*) with no blame assigned.
- **Clear structure:** Moves logically from context → identified issues → unresolved matter → proposed actions — appropriate for a consolidated report.
...
|---|---|---|
| 1. *"Offices should try to sort out the issues with the forms as soon as they can."* | Vague; informal; no deadline or responsible party | *All offices are requested to confirm adoption of the revised form no later than [date].* |
| 2. *"The meeting didn't really come to any conclusions about what to do next."* | Informal; passive observation | *No follow-up action was agreed during the meeting, and further clarification is required before next steps can be assigned.* |
| 3. *"This is very important and everyone needs to take it seriously."* | Emotive; vague; no specific action | *Compliance with the requirements set out in Circular 2024/12 is mandatory. Offices that have not yet acknowledged receipt are asked to do so immediately.* |
...
- **R:** *"A meeting needs to happen soon."* → *A coordination meeting is proposed for [date] to address outstanding implementation concerns.*
- **R:** *"Offices should respond and let us know what is going on."* → *Offices are asked to submit a brief implementation status update by [date].*
- **D** *"The situation is not ideal but hopefully it will get better."* — evaluative; no informational value

**Clean revised version:**
> Several implementation concerns have emerged since the distribution of the revised guidelines. Terminology in the updated guidelines is not yet being applied consistently across all offices, and a number of offices continue to use an outdated applicant form.
>
...
| Exercise Type | What to Assess | Notes |
|--------------|----------------|-------|
| Identify / classify | Accuracy of analysis | Accept paraphrased explanations that show correct understanding |
| Match | Accuracy | One correct answer per item unless noted |
| Sequence sentences | Logical order | Accept alternative sequences with convincing reasoning |
| Add coordinators / connectors | Grammatical correctness + logical appropriateness | Multiple acceptable answers — require explanation |
| Replace repetition | Grammatical correctness + natural register | "It / they / these / such + noun" patterns |
| Rewrite for clarity | Segmentation, connector use, sentence control | Assess against the three tools: coordination, subordination, segmentation |
| Rewrite for tone | Softening strategies used + audience match | Three softening strategies minimum in homework tasks |
| Guided writing (~150 words) | Structure + diplomatic language + specific references + next step | All four elements required for full marks |
...
|--------|-------------|-----------|
| 1 | Treating segmentation as "making sentences shorter" | Segmentation is about separating logical jobs, not minimising length |
| 1 | Using "although" for equal contrast | Use "although" only when the contrast idea is secondary; use "but" or "however" for equal contrast |
| 2 | Softening to the point of vagueness | Softening must preserve clarity — the reader must still understand the purpose |
| 2 | Using the same level of formality for all audiences | Tone must vary; internal ≠ interagency ≠ foreign office |
| 3 | Adding connectors without changing sentence structure | Cohesion requires structural revision, not just connector insertion |
| 4–7 | Leaving next-step statements implicit | High-level emails must always state what happens after the reply is received |
| 8–11 | Passive-heavy procedures | Procedures should use active imperative verbs |
